...
def cwicz_wymowe():
    ...
    # Tworzenie funkcji do nagrywania głosu ----------------------------------------------------------------------------------------------------------------------------
    def nagraj_glos():
        global random_key, random_value
        result_label.configure(text="......🎙.......")
        result_label.update()  # Wymuszenie natychmiastowego odświeżenia GUI
        ...
        try:
            ...
            CTkMessagebox(title="Błąd", message="Niestety nie rozpoznaliśmy mowy. Spróbuj ponownie.", icon="cancel")
            result_label.configure(text="---Twoja odpowiedź---")
        except sr.RequestError as e:
            print(f"Błąd połączenia z usługą rozpoznawania mowy: {e}")
            CTkMessagebox(title="Błąd", message=f"Błąd połączenia z usługą rozpoznawania mowy: {e}", icon="cancel")
            result_label.configure(text="---Twoja odpowiedź---")
    ...
    def nowe_slowko():
        ...
        print(random_key)
        print(random_value)
        # Wprowadzenie losowego klucza ze słownika do label_dla_random_key
        label_dla_random_key.configure(text=random_key)
    ...
    # Utwórz pole tekstowe
    ctk.CTkLabel(upper_frame,
                         text='W tym oknie przećwiczysz angielską wymowę słów podawanych dla Ciebie w języku polskim. Gdy pojawi się polskie tłumaczenie, naciśnij przycisk "⏵", a następnie wypowiedz angielski odpowiednik. Aplikacja pokaże, czy podane zostało poprawne słowo angielskie',
                         wraplength=450, font=("Helvetica", 14), anchor="center").grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
    ...
    # Dodawanie labela, w którym znajdzie się random_key
    label_dla_random_key = ctk.CTkLabel(new_window,
                                        text="test",
                                        font=("Helvetica", 16),
                                        anchor="center",
    ...
    # Utwórz ramkę, w której będzie pokazywany wynik
    result_frame = ctk.CTkFrame(new_window, height=40, width=200)
    result_frame.grid(row=1, column=3, padx=5, pady=5, sticky="nsew")
    result_frame.grid_propagate(False)  # Wyłączenie automatycznego dostosowywania rozmiaru

    # Skonfiguruj siatkę w result_frame, aby środkować zawartość
    result_frame.grid_rowconfigure(0, weight=1)
    result_frame.grid_columnconfigure(0, weight=1)

    # Utwórz pole tekstowe dla Twojej wypowiedzi
    ...
    play_button = ctk.CTkButton(new_window, text="⏵ (enter)", font=("Helvetica", 14),text_color="white", command=nagraj_glos)
    play_button.grid(row=3, column=3, pady=10)

    # Dodawanie przycisku "Nowe słówko"
    new_button = ctk.CTkButton(new_window, text="Nowe słówko ⮌", font=("Helvetica", 14), text_color="white", command=nowe_slowko)
...
def tlumacz_ze_sluchu():
    ...
    # Utwórz pole tekstowe i dodaj je do wewnętrznej ramki (inner_frame)
    ctk.CTkLabel(upper_frame,
                         text='W tym oknie usłyszysz angielskie słówko lub zwrot wypowiedziane przez lektora. Twoim zadaniem będzie zaznaczenie prawidłowego polskiego tłumaczenia. Wystarczy kliknąć na przycisk, żeby sprawdzić wynik.\nW razie potrzeby możesz odtworzyć słówko ponownie.',
    ...
    middle_frame.grid_propagate(False)  # Wyłączenie automatycznego dostosowywania rozmiaru
...
# Powrót do menu
#
# Kiedy kliknę w dobre zdjęcie:
# - po 0.5s odtwarza się angielskie słówko
# - w tym samym momencie pojawia się okienko gratulacyjne
#
# Kiedy kliknę w złe zdjęcie:
# - odtwarza się dźwięk systemowy sugerujący błąd
...
#         threading.Thread(target=read_word, args=(random_value,)).start()
    ...
    # Utwórz pole tekstowe i dodaj je do wewnętrznej ramki (inner_frame)
    ctk.CTkLabel(upper_frame,
                         text="W tym oknie sprawdzimy, czy prawidłowo kojarzysz tłumaczenie usłyszanego słowa z przypisanym do niego zdjęciem. Odsłuchaj polskie tłumaczenie, a następnie kliknij na jeden z trzech obrazków, żeby przekonać się, czy masz rację.",
                         wraplength=450, font=("Helvetica", 14), anchor="center").grid(row=0, column=0, padx=10, pady=10, sticky="nsew")


    # Dodawanie przycisku "Odtwórz"
    play_button = ctk.CTkButton(new_window, text="♫ Odtwórz ♫", font=("Helvetica", 13), text_color="white")
    play_button.grid(row=1, column=0, padx=10, pady=10, sticky="nsew")





    # Ramka na zdjęcia + 3 przyciski--------------------------------------------------------------------------------------------------------
    # Stwórz ramkę do przycisków z obrazami
    frame = ctk.CTkFrame(new_window, width=300, height=150)
    frame.grid(row=2, column=0, padx=20, pady=10, sticky="nsew", columnspan = 3)
    frame.grid_propagate(False)  # Wyłączenie automatycznego dostosowywania rozmiaru

    # Skonfiguruj wiersze i kolumny w ramach frame
    frame.grid_rowconfigure(1, weight=2)  # Środkowy wiersz z przyciskami
    ...
    button1 = ctk.CTkButton(frame, text="test1", width=150, height=150, fg_color="transparent")
    button1.grid(row=2, column=0, padx=10, pady=10, sticky="nsew")
    button1.grid_propagate(False)  # Wyłączenie automatycznego dostosowywania rozmiaru

    button2 = ctk.CTkButton(frame, text="test2", width=150, height=150, fg_color="transparent")
    button2.grid(row=2, column=1, padx=10, pady=10, sticky="nsew")
    button2.grid_propagate(False)  # Wyłączenie automatycznego dostosowywania rozmiaru

    button3 = ctk.CTkButton(frame, text="test3", width=150, height=150, fg_color="transparent")
    button3.grid(row=2, column=2, padx=10, pady=10, sticky="nsew")
    button3.grid_propagate(False)  # Wyłączenie automatycznego dostosowywania rozmiaru
    ...
    def losuj_trojki():
        ...
        random_key = random.choice(list(slownik.keys()))
        random_value = slownik[random_key]
        random_key1 = random_key
        random_value1 = random_value

        random_key = random.choice(list(slownik.keys()))
        random_value = slownik[random_key]
        random_key2 = random_key
        random_value2 = random_value

        random_key = random.choice(list(slownik.keys()))
        random_value = slownik[random_key]
        random_key3 = random_key
        random_value3 = random_value

        print(f'random_key1: {random_key1}')
        print(f'random_value1: {random_value1}')
        print(f'random_key2: {random_key2}')
        print(f'random_value2: {random_value2}')
        print(f'random_key3: {random_key3}')
        print(f'random_value3: {random_value3}')

        random_value = random.choice([random_value1, random_value2, random_value3])
        threading.Thread(target=read_word, args=(random_value,)).start()
        ...
        new_image1 = fetch_random_image(random_value1)
        print(f'new_image1: {new_image1}')
        new_image2 = fetch_random_image(random_value2)
        print(f'new_image2: {new_image2}')
        new_image3 = fetch_random_image(random_value3)
        print(f'new_image3: {new_image3}')

        if new_image1:
            resized_image1 = new_image1.resize((150, 150), Image.LANCZOS)
            print(f'resized_image1: {resized_image1}')
            image1 = CTkImage(resized_image1)
            button1.configure(image=image1)
            button1.image = image1
            print(f'button1 image: {button1.image}')

        if new_image2:
            resized_image2 = new_image2.resize((150, 150), Image.LANCZOS)
            print(f'resized_image2: {resized_image2}')
            image2 = CTkImage(resized_image2)
            button2.configure(image=image2)
            button2.image = image2
            print(f'button2 image: {button2.image}')

        if new_image3:
            resized_image3 = new_image3.resize((150, 150), Image.LANCZOS)
            print(f'resized_image3: {resized_image3}')
            image3 = CTkImage(resized_image3)
            button3.configure(image=image3)
            button3.image = image3
            print(f'button3 image: {button3.image}')

    # Dodawanie przycisku "Nowe słówko"
    new_button = ctk.CTkButton(new_window, text="Nowe słówko ⮌", font=("Helvetica", 13), text_color="white",
                               command=losuj_trojki)
    new_button.grid(row=1, column=1, padx=10, pady=10, sticky="nsew")
...
button8 = ctk.CTkButton(frame, text="Zakończ", height=50, fg_color="#99a7bd", text_color="black", command=confirm_exit)
