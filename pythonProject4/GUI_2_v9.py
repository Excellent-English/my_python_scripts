...
def uczsie2(bg_color):
    ...
    def initialize_random_word():
        ...
        ctk.CTkLabel(lower_left_frame,
                     text=random_key,
                     wraplength=250, font=("Helvetica", 14), anchor="center").grid(row=2, column=0, padx=10, pady=10,
                                                                                   sticky="n")

    initialize_random_word()
    ...
    def click_on_check():
        global random_key, random_value
        print("Oto kolejno random_key i random_value:")
        print(random_key)
        print(random_value)
        my_value = entry_value.get()  # Pobierz wartość wpisaną przez użytkownika
        print("A o to my_value: ")
        print(my_value)
        if my_value == random_value:
            CTkMessagebox(title="Wspaniale", message="Udało Ci się poprawnie przetłumaczyć słówko", icon="question")
            ...
            # Utwórz etykietę i przypisz ją do zmiennej
            label = ctk.CTkLabel(lower_left_frame,
                                 text=random_key,
                                 wraplength=250, font=("Helvetica", 14), anchor="center")
            label.grid(row=2, column=0, padx=10, pady=10, sticky="n")

        else:
            CTkMessagebox(title="Źle", message="Może następnym razem się uda", icon="question")

    # Dodawanie przycisku "Sprawdź >>"
    verify_button = ctk.CTkButton(new_window, text="Sprawdź >>", font=("Helvetica", 14), width=150, text_color="white", command=click_on_check)
    ...
    def click_to_check():
        ...
        if moja_odp == random_value:
            ...
            if len(nowy_slownik_do_nauki) > 0:
                ...
                # pole_do_wpisywania_propozycji.delete(0, 'end')  # Wyczyść pole Entry
                CTkMessagebox(title="Sukces!", message="Brawo! Wykonałeś dobrą robotę!", icon="check")
            ...
            CTkMessagebox(title="Źle", message="Może następnym razem się uda", icon="cancel")



    # Dodawanie przycisku "Sprawdź >>"
    verify_button = ctk.CTkButton(new_window, text="Sprawdź >>", font=("Helvetica", 14), width=150, text_color="white", command=click_to_check)
    verify_button.grid(row=5, column=1, pady=5)
    upper_frame.grid_propagate(False)  # Wyłączenie automatycznego dostosowywania rozmiaru
